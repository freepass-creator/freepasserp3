...
export const SHEET_CONFIGS = {
  ...
  rentzone: { sheet_id: '1IFV4_rNq4hW_KornQpz1ppBWbziCyklTo6oOH0BaUc8', tab_name: '시트1', provider_code: 'PT-0014', label: '렌트존 (PT-0014)', schema: 'rentco' },
  ...
  ecorent: { sheet_id: '1F2l4uTUqm_n3byL7d4fIkreCdpt4Vo5tiarMSh5iOmQ', tab_name: '시트1', provider_code: 'RP032', label: '에코렌트카 (RP032)', schema: 'rentco' },
};
...
const PARTNER_NAME_TO_CODE = {
  ...
  '이안카': 'RP031',
  '에코렌트카': 'RP032', '에코렌터카': 'RP032', '에코': 'RP032',
};
...
function parseRentCoRow({ row, headers, absRow, photoLinkMap, providerCode, sheetId, nowMs }) {
  ...
  const product = {
    ...
    price: {},
    // 시트에 있는 매물 단위 값 — 정책(policy) 매칭 실패/불일치와 무관하게 항상 이 값 우선 사용
    sheet_meta: {
      ...
      ins_injury: safeGet(row, colIdx('대인')),
      ins_property: safeGet(row, colIdx('대물')),
