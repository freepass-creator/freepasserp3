 ...
 *
 * ⚠ 실제 Firebase 와 완전히 분리된 가짜 데이터. 절대 실데이터/실코드가 아님.
 *   demo.js 의 인메모리 DB 로 로드되어 watchCollection 대신 화면에 흐른다.
 *
 * 레코드 형태는 pages/search.js·product.js·workspace.js 가 읽는 필드 기준으로 맞춤:
 *   - product.price = { '기간': {rent, deposit} } 맵 (search.js getPricePairForPeriod)
 *   - product.vehicle_status = SSOT 어휘(출고가능 등, core/vehicle-status.js)
 *   - room.product_uid = product._key, room.car_number/model 필요(workspace.js)
 *   - 채팅은 최상위 messages/{roomId} 컬렉션에서 구독(workspace.js) — inline 아님
 */

const SP = 'DEMO-SP';          // 데모 공급사 코드
const CH = 'DEMO';             // 데모 영업채널 코드

/* 기간별 대여료 맵 생성 — 짧을수록 월세↑ (36개월 기준가 base) */
function priceMap(base, dep) {
  const r = (mult) => Math.round((base * mult) / 1000) * 1000;
  return {
    '12': { rent: r(1.14), deposit: dep },
    '24': { rent: r(1.06), deposit: dep },
    '36': { rent: base,    deposit: dep },
    '48': { rent: r(0.95), deposit: dep },
  };
}
...
const PRODUCTS = {
  'demo-p1': {
    product_uid: 'demo-p1', product_code: 'DEMO-0001', product_type: '무보증',
    ...
    price: priceMap(690000, 0), deposit_free: true, credit_grade: '무관',
    vehicle_status: '출고가능', status: 'available',
  ...
  'demo-p2': {
    product_uid: 'demo-p2', product_code: 'DEMO-0002', product_type: '무보증',
    ...
    price: priceMap(430000, 0), deposit_free: true, credit_grade: '무관',
    vehicle_status: '출고가능', status: 'available',
    provider_company_code: SP, partner_code: SP,
    policy_code: 'DEMO-POL-N', policy_name: '무보증 표준',
  ...
  'demo-p3': {
    product_uid: 'demo-p3', product_code: 'DEMO-0003', product_type: '일반',
    ...
    price: priceMap(320000, 640000), deposit_free: false, credit_grade: '무관',
    vehicle_status: '즉시출고', status: 'available',
    provider_company_code: SP, partner_code: SP,
  ...
  'demo-p4': {
    product_uid: 'demo-p4', product_code: 'DEMO-0004', product_type: '무심사',
    ...
    price: priceMap(560000, 1120000), deposit_free: false, credit_grade: '무관(무심사)',
    vehicle_status: '출고가능', status: 'available',
    provider_company_code: SP, partner_code: SP,
  ...
  'demo-p5': {
    product_uid: 'demo-p5', product_code: 'DEMO-0005', product_type: '일반',
    ...
    price: priceMap(390000, 780000), deposit_free: false, credit_grade: '무관',
    vehicle_status: '출고가능', status: 'available',
    provider_company_code: SP, partner_code: SP,
    policy_code: 'DEMO-POL-G', policy_name: '일반 표준',
  ...
  'demo-p6': {
    product_uid: 'demo-p6', product_code: 'DEMO-0006', product_type: '무보증',
    maker: '기아', model: '봉고3', sub_model: '봉고3 1톤', trim_name: '킹캡',
    year: 2021, car_number: '90머 7725', mileage: 63000,
    fuel_type: '디젤', ext_color: '화이트', int_color: '그레이',
    price: priceMap(410000, 0), deposit_free: true, credit_grade: '무관',
    vehicle_status: '출고가능', status: 'available',
    provider_company_code: SP, partner_code: SP,
    policy_code: 'DEMO-POL-N', policy_name: '무보증 표준',
...
/* ── 샘플 대화방 1개 (업무소통 체험용) — 차량정보 필드 포함 ── */
const T0 = () => Date.now();
const ROOMS = {
  'demo-room1': {
    agent_uid: 'demo-agent', agent_channel_code: CH, agent_code: 'DEMO',
    provider_company_code: SP,
    product_uid: 'demo-p1', product_code: 'DEMO-0001',
    car_number: '12가 3456', maker: '기아', model: '카니발', sub_model: '4세대 카니발',
    last_message: '네, 계약 진행 도와드릴게요!',
    last_message_at: T0() - 1000 * 60 * 12,
    unread_for_agent: 0, unread_for_provider: 0,
    created_at: T0() - 1000 * 60 * 60 * 3,
  },
};

/* ── 채팅 메시지 — 최상위 messages/{roomId} 컬렉션 (workspace.js 구독 위치) ── */
const MESSAGES = {
  'demo-room1': {
    m1: { text: '카니발 노블레스 무보증으로 가능할까요?', sender_role: 'agent',    sender_uid: 'demo-agent',    sender_code: 'DEMO',    sender_name: '체험 영업자',   created_at: T0() - 1000 * 60 * 60 * 3 },
    m2: { text: '네 가능합니다. 보증금 없이 월 69만원, 36개월 조건이에요.', sender_role: 'provider', sender_uid: 'demo-provider', sender_code: 'DEMO-SP', sender_name: '샘플모빌리티', created_at: T0() - 1000 * 60 * 60 * 2 },
    m3: { text: '고객 서류 준비되면 계약 넣겠습니다.', sender_role: 'agent',    sender_uid: 'demo-agent',    sender_code: 'DEMO',    sender_name: '체험 영업자',   created_at: T0() - 1000 * 60 * 20 },
    m4: { text: '네, 계약 진행 도와드릴게요!', sender_role: 'provider', sender_uid: 'demo-provider', sender_code: 'DEMO-SP', sender_name: '샘플모빌리티', created_at: T0() - 1000 * 60 * 12 },
  },
};
