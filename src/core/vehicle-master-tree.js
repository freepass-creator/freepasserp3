 ...
 *
 *   제조사(maker) → 모델(model_root) → 세부모델(catalog) → 모델구분(variant) → 트림(trim)
 *
 * trim 문자열 형태: "{트림} {연료} {배기량} [T] [인승] [구동]"
 *   "인스퍼레이션 가솔린 1.6"      → 트림 "인스퍼레이션" / 모델구분 "가솔린 1.6"
 *   "프레스티지 디젤 2.2 9인승"     → 트림 "프레스티지"   / 모델구분 "디젤 2.2 9인승"
 *   "가솔린 2.5 T AWD"(제네시스)   → 트림 "(기본)"       / 모델구분 "가솔린 2.5 T AWD"
 *   "라이트 스탠다드 EV"           → 트림 "라이트"       / 모델구분 "스탠다드 EV"
 *
 * 분해 규칙: 토큰을 뒤에서부터 보아 '스펙 토큰'(연료·배기량·터보·인승·구동·배터리)이
 *   연속되는 최대 꼬리 = 모델구분, 그 앞 나머지 = 트림. (적용·매칭 없이 구성만)
 */
